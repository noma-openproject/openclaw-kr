...
- **Node.js 22.12 이상** — [nodejs.org](https://nodejs.org)에서 LTS 버전 다운로드 (설치 후 앱이 자동으로 찾습니다)
...
1. **Node.js가 설치되어 있는지 확인**: 터미널/명령 프롬프트에서 `node --version` 실행 → v22.12 이상이어야 합니다
2. Node.js 미설치 시: [nodejs.org](https://nodejs.org)에서 LTS 버전 설치 후 앱 재시작
3. 앱을 완전히 종료한 후 다시 실행해보세요 (첫 실행 시 최대 60초 소요될 수 있습니다)
